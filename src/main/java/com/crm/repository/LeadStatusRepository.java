...
public interface LeadStatusRepository extends JpaRepository<LeadStatusMaster, Long> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT s FROM LeadStatusMaster s WHERE s.active = true AND s.statusName IS NOT NULL AND TRIM(s.statusName) != '' AND LOWER(TRIM(s.statusName)) != 'null' AND (s.userIdFk = :userIdFk OR s.userIdFk IS NULL)")
